...
Максимальное значение этого выражения определяет функцию качества \\(k\\)-го блока (без учёта вычитания \\(\text{ncp\_prior}\\)). Если \\(a=0\\), мы получаем уже известную функцию для кусочно-постоянной модели \eqref{fitness_function}.
